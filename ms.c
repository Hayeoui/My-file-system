		...
		else
		{
			if(access(file_name, F_OK) == 0)
			{
				...
				else // co_num == 18(cygwin 명령어 실행)
				{
					...
					{
						final_func(&superblock, inode_list);
						exit(0);
					}
					else
					{
...
int update_mymkfs(super *sp, inode *ip, inode **now_dp)
{
	...
	for(i=0; i<64; ++i)
		fread(&(sp->inode_use[i]), sizeof(sp->inode_use[i]), 1, ofp);

	for(i=0; i<128; ++i)
		fread(&(sp->datablock_use[i]), sizeof(sp->datablock_use[i]), 1, ofp);
	...
	for(i=0; i<1024; ++i)
	{
		if(dbit[i] == 1)
		{
			fread(&type[k], sizeof(char), 1, ofp);
			k++;
		}
		else
			;
	...
	for(i=0; i<1024; ++i)
	{
		if(dbit[i] == 1)
		{
			dblock[i] = (data *)malloc(sizeof(data));

			if(type[k] == 'd')
				fread(dblock[i]->con.dc, sizeof(dblock[i]->con.dc), 1, ofp);
			else if(type[k] == 'f')
				fread(dblock[i]->con.fc, sizeof(dblock[i]->con.fc), 1, ofp);
			else
				fread(dblock[i]->con.ind, sizeof(dblock[i]->con.ind), 1, ofp);

			fread(&(dblock[i]->next), sizeof(data *), 1, ofp);
			fread(&(dblock[i]->down), sizeof(data *), 1, ofp);
			fread(&(dblock[i]->link), sizeof(data *), 1, ofp);
			fread(&(dblock[i]->parent), sizeof(int), 1, ofp);
			fread(&(dblock[i]->d_num), sizeof(int), 1, ofp);
			fread(&(dblock[i]->type), sizeof(char), 1, ofp);
	...
	for(i=0; i<1024; ++i)
	{
		...
		{
			for(j=0; j<3; ++j)
				fread(&dnl[k][j], sizeof(int), 1, ofp);
			k++;
		}
	}
	...
	for(i=0; i<1024; ++i)
	{
		if(dbit[i] == 1)
		{
			...
			if(tmp == 0)
				...
				dblock[i]->down = dblock[tmp];
			tmp = dnl[k][1];
			if(tmp == 0)
				dblock[i]->next = NULL;
			else
				dblock[i]->next = dblock[tmp];
			tmp = dnl[k][2];
			if(tmp == 0)
				dblock[i]->link = NULL;
			else
				dblock[i]->link = dblock[tmp];
	...
	if(n != 0)//처음이 아닌 경우 -와 *를 출력
	{
		...
		else//처음이 /가 아닌 경우 이름을 찾아 출력
		{
			for(i=0; dblock[ip[now->parent].db]->con.dc[i][0]!='\0'; i++)
				if(now->d_num == ip[(100*(dblock[ip[now->parent].db]->con.dc[i][0]-48)+10*(dblock[ip[now->parent].db]->con.dc[i][1]-48)+(dblock[ip[now->parent].db]->con.dc[i][2]-48))].db)
					break;
...
void final_func(super *sp, inode *ip)
{
	...
	for(i=0; type[i]!='\0'; ++i)
	{
		fwrite(&type[i], sizeof(char), 1, ofp);
	}
	...
	for(i=0; i<1024; ++i)
	{
		...
		else
		{
			...
			if(dblock[i]->next == NULL)
				dnl[k][1] = 0;
			else
			{
				temp = dblock[i]->next->d_num;
				dnl[k][1] = temp;
			}
			...
			if(type[k] == 'd')
				fwrite(dblock[i]->con.dc, sizeof(dblock[i]->con.dc), 1, ofp);
			else if(type[k] == 'f')
				fwrite(dblock[i]->con.fc, sizeof(dblock[i]->con.fc), 1, ofp);
			else
				fwrite(dblock[i]->con.ind, sizeof(dblock[i]->con.ind), 1, ofp);

			fwrite(&(dblock[i]->next), sizeof(data *), 1, ofp);
			fwrite(&(dblock[i]->down), sizeof(data *), 1, ofp);
			fwrite(&(dblock[i]->link), sizeof(data *), 1, ofp);
			fwrite(&(dblock[i]->parent), sizeof(int), 1, ofp);
			fwrite(&(dblock[i]->d_num), sizeof(int), 1, ofp);
			fwrite(&(dblock[i]->type), sizeof(char), 1, ofp);
	...
	{
		for(j=0; j<3; ++j)
			fwrite(&dnl[i][j], sizeof(int), 1, ofp);
	}
